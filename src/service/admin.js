...
const page = ({ flash, basePath, procs, pools, mon, qr, sms, platform, trace, warnings, logs }) => {
  ...
  const claimHint = liteActive
    ? `当前 Lite 池：${litePools.map(item => item.label).join(' / ')}，支持自动领取和手动领取。`
    : '当前没有池运行在 Lite 模式，领取按钮不会执行。'
...
<section class="card full"><h2>会员特权调度</h2><div class="hint">${esc(claimHint)}<br>可附加 <code>?format=json</code> 直接获取 API 响应。</div><div class="actions">${pools.map(item => `<form method="post" action="${root(basePath)}/kugou/vip/claim"><input type="hidden" name="pool" value="${esc(item.pool)}"><button type="submit"${item.platform === 'lite' ? '' : ' disabled'} class="${item.pool === 'general' ? 'ghost' : ''}">领取${esc(item.label)} Lite 会员</button></form>`).join('')}<form method="post" action="${root(basePath)}/kugou/vip/claim-all"><button class="danger" type="submit"${liteActive ? '' : ' disabled'}>批量执行</button></form></div><div class="table-wrap"><table><thead><tr><th>池分区</th><th>平台</th><th>上次下发</th><th>执行结果</th></tr></thead><tbody>${pools.map(item => `<tr><td>${esc(item.label)}</td><td>${esc(platformDisplayLabel(item.platform))}</td><td>${esc(time(item.state?.lastClaimAt))}</td><td>${esc(resultText(item.state?.lastClaimResult))}</td></tr>`).join('')}</tbody></table></div></section>
...
export default async (c) => {
  ...
  return c.html(page({ flash, basePath, procs, pools, mon, qr, sms, platform, trace: getKugouUpstreamTrace(), warnings, logs }))
}
